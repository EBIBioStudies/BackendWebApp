...
import uk.ac.ebi.biostd.webapp.shared.dto.submission.DataModuleMeta;
import uk.ac.ebi.biostd.webapp.shared.dto.submission.FileAttachmentMeta;
import uk.ac.ebi.biostd.webapp.shared.dto.submission.SubmissionMeta;
...
//  form.setWidth("500px");
  ...
  panel.add( new Hidden(SubmissionConstants.SUBMISSON_STATUS, SubmissionStatus.UPDATE.name() ) );
...
//  btPan.setWidget(0, 1, addBt);
 ...
 private class DMInfoPanel extends CaptionPanel
 {
  ...
  private final CheckBox dscCB = new CheckBox();
  private final CheckBox fileCB = new CheckBox();
  
  private final Hidden statusInput = new Hidden();
  
  private SubmissionStatus status = SubmissionStatus.KEEP;
...
//   layout.setWidget(3, 0, upload);
  ...
  private void updateStatus()
  {
   if( status != SubmissionStatus.DELETE )
   {
    if( dscCB.getValue() || fileCB.getValue() )
     status = SubmissionStatus.UPDATE;
    else
     status = SubmissionStatus.KEEP;
   }
 ...
 {
  private final Hidden origId = new Hidden();
  
  private final TextBox id;
  private final CheckBox idCB = new CheckBox();
  ...
  private final FileAttachmentMeta fAtMeta;
