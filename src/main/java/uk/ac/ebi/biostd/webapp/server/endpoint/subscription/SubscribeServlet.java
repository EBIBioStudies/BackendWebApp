...
public class SubscribeServlet extends ServiceServlet
{
 ...
 private void listSubscriptions( HttpServletResponse response, boolean json, Session sess) throws IOException
 {
  ...
  try
  {
   Collection<TagSubscription> subss=BackendConfig.getServiceManager().getTagManager().listSubscriptions(sess.getUser());
 ...
 private void subscribe(ParameterPool params, Response resp, Session sess, boolean subsc) throws IOException
 {
  ...
  {
   if( subsc )
    BackendConfig.getServiceManager().getTagManager().subscribeUser(tagName, classifierName, sess.getUser());
   else
    BackendConfig.getServiceManager().getTagManager().unsubscribeUser(tagName, classifierName, sess.getUser());
