...
public class FormSubmitServlet extends ServiceServlet
{
 ...
 @Override
 protected void service(HttpServletRequest req, HttpServletResponse response, Session sess) throws ServletException, IOException
 {
  if( ! req.getMethod().equalsIgnoreCase("POST") )
  {
   respond(response, HttpServletResponse.SC_METHOD_NOT_ALLOWED, "Method "+req.getMethod()+" is not allowed");
   return;
  }
  ...
  if( fmtStr != null && fmtStr.trim().length() > 0 )
  {
   ...
   {
    respond(response, HttpServletResponse.SC_BAD_REQUEST, "FAIL Invalid 'type' attribute value: '"+fmtStr+"'");
    return;
   }
  }
  ...
  if( fmt == null )
  {
   respond(response, HttpServletResponse.SC_BAD_REQUEST, "FAIL can't determine file type. Define 'type' attribute");
   return;
  }
